...
// Represents gxt default container.


public class GxtDefaultContainer extends SimpleContainer {
    ...
    public static final double DEFAULT_PREFERRED_HEIGHT = 1;
    public static final double DEFAULT_PREFERRED_MARGIN = 10;
    public static final double DEFAULT_PREFERRED_WIDTH = 1;
    ...
    public GxtDefaultContainer(IsWidget widget) {
        super();
        setWidget(widget);
    ...
    public GxtDefaultContainer setMaxHeight(double maxHeight) {
        this.maxHeight = maxHeight;
        return this;
    }

    public GxtDefaultContainer setMaxWidth(double maxWidth) {
        this.maxWidth = maxWidth;
        return this;
    }

    public GxtDefaultContainer setMinHeight(double minHeight) {
        this.minHeight = minHeight;
        return this;
    }

    public GxtDefaultContainer setMinWidth(double minWidth) {
        this.minWidth = minWidth;
        return this;
    }

    public GxtDefaultContainer setPreferredHeight(double preferredHeight) {
        this.preferredHeight = preferredHeight;
        return this;
    }

    public GxtDefaultContainer setPreferredMargin(double preferredMargin) {
        this.preferredMargin = preferredMargin;
        return this;
    }

    public GxtDefaultContainer setPreferredWidth(double preferredWidth) {
    ...
    protected void doLayout() {
        if (widget != null) {
            ...
            // determine if the widget is clean by checking if current bounds
            // are clean
            final Rectangle currentBounds = element.getBounds(true);
            final boolean cleanWidget = cleanBounds == null || (currentBounds.getX() == cleanBounds.getX()
                    && currentBounds.getY() == cleanBounds.getY() && currentBounds.getWidth() == cleanBounds.getWidth()
            ...
            if (cleanWidget) {
                ...
                // the widget needs to use relative positioning for container
                // scrolling to work
                element.makePositionable(false);

                // enable container scrolling appropriate depending on type of
                // device
                if (GXT.isTouch()) {
                    container.getStyle().setOverflow(Overflow.SCROLL);
                } else {
                    // widgets wanting 100% must handle scrolling themselves to
                    // avoid extraneous scroll bars
                ...
                Scheduler.get().scheduleFinally(new Command() {
                    @Override
                    public void execute() {
                        // after the layout has finished, fetch the adjusted
                        // widget size (including any possible overflow &
                        // scrollbars)
                        ...
                        // if the actual size differs from the calculated size,
                        // position the widget again because it's not right
                        if (adjustedWidthPx != widgetWidthPx || adjustedHeightPx != widgetHeightPx) {
                            ...
                            // layout the widget at its recalculated position &
                            // size
                            applyLayout(widget,
                                    new Rectangle(adjustedLeftPx, adjustedTopPx, adjustedWidthPx, adjustedHeightPx));
                        }

                        Scheduler.get().scheduleFinally(new Command() {
                            @Override
                            public void execute() {
                                // after the layout has finished, preserve the
                                // clean bounds for next time
                ...
                // once the widget is dirty, ensure it can't ever be clean again
                // even if the user restores original bounds
            ...
            Scheduler.get().scheduleFinally(new Command() {
                @Override
                public void execute() {
                    // after the layout has finished, focus may be off screen or
                    // covered by a virtual keyboard
                    // so we need to ensure focus is on screen or the user may
                    // get confused
